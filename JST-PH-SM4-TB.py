...
for i in range(len(PH_pins)):
    f = footgen.Footgen(PH_basename.format(PH_EntryType,PH_pins[i]))
    f.pins = PH_pins[i] 
    f.height = PH_dimA[i]
...
# fxn should check the f.rowofopads.whichway value to determine default location
    ...
    diamond_ycenter = f.padwidth/2-7.5
    diamond_size = 0.5
...
PH_basename = "JST-{0}{1}B-PH-SM4-TB"
PH_EntryType = str("S") # Side entry type
